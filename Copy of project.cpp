...
void brute(int matrix[maxVertices][maxVertices+10], int vertices, int edges, 
		   ...
		   int sort, int test, int best, int verify, bool &finished, int mask[]);
...
void output(int matrix[maxVertices][maxVertices+10], int vertices, 
			double seconds, string fileString);
void recurse(int matrix[maxVertices][maxVertices+10], int vertices, int edges,
			 ...
			 int count[], int mask[], int i, int j);
...
int main(int argc, char* argv[])
{
	...
	Graph >> edges; /* this number is thrown away */
	Graph >> node1;
	Graph >> node2;
	...
	output(matrix, vertices, seconds, fileString);
...
void brute(int matrix[maxVertices][maxVertices+10], 
		   ...
		   int sort,
		   int test, int best,
		   int verify, bool &finished, int mask[])
{
...
void output(int matrix[maxVertices][maxVertices+10], 
			int vertices, double seconds, string fileString)
{
		...
		if(i==0)
		{
			for(count[i]=count[i+1]+1; count[i]<vertices-i; count[i]++)
			{
				...
				brute(matrix, vertices, edges, required, notRequired, 
					maybe, maybeNot, sort, test, best, verify, finished, mask);
				mask[count[i]]=0;
				if(finished==1) break;
		}	}
		else if(i==j)
...
{
	finished=1;
	for(int i=0; i<vertices; i++) /* check to see if finished */
	{
		if(matrix[i][required]==0 && 
			matrix[i][notRequired]==0 && 
			matrix[i][maybe]==0 &&
			matrix[i][maybeNot]==0)
			finished=0;
}	}
